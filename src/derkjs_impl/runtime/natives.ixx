...
export namespace DerkJS {
    ...
    [[nodiscard]] auto native_number_to_fixed(ExternVMCtx* ctx, [[maybe_unused]] PropPool<Value, Value>* props, int argc) -> bool {
        ...
        if (auto fixed_string_p = ctx->heap.add_item(ctx->heap.get_next_id(), std::make_unique<DynamicString>(
            ctx->base_protos.at(static_cast<unsigned int>(BasePrototypeID::str)),
            ctx->base_protos.at(static_cast<unsigned int>(BasePrototypeID::extra_length_key)),
            result
        )); fixed_string_p) {
            ctx->stack.at(passed_rsbp - 1) = Value {fixed_string_p};
            return true;
        }

        ctx->status = VMErrcode::bad_heap_alloc;
        std::println(std::cerr, "Number.toFixed: failed heap allocation of result string.");
    ...
    [[nodiscard]] auto native_error_ctor(ExternVMCtx* ctx, [[maybe_unused]] PropPool<Value, Value>* props, int argc) -> bool {
        const auto passed_rsbp = ctx->rsbp;
