...
export module runtime.number;

import runtime.value;

namespace DerkJS {
    export class NumberBox : public ObjectBase<Value> {
    private:
    ...
    public:
        NumberBox(const Value& temp_value, ObjectBase<Value>* prototype_p)
        : m_properties {}, m_prototype {prototype_p, std::to_underlying(AttrMask::defaults) | std::to_underlying(AttrMask::property)}, m_value {temp_value}, m_flags {std::to_underlying(AttrMask::defaults)} {}
        ...
        [[nodiscard]] auto operator==(const ObjectBase<Value>& other) const noexcept -> bool override {
            ...
            if (const auto other_as_number_box_p = dynamic_cast<const NumberBox*>(&other); other_as_number_box_p) {
                return m_value == other_as_number_box_p->get_native_value();
            }
